    ...
    public static class UserInterface
    {
        ...
        public static string LocaleAllAlbums(List<Album> albums = null)
        {
            ...
            do
            {
                ...
                for (int j = 0; j < songsToPrint.Count; j++)
                {
                    Console.WriteLine($"{j + 1}. {songsToPrint[j].AlbumName}");
                }
                Console.WriteLine("\n0. Next page");
                Console.WriteLine("H. Torna alla homepage");
                SongPlayer();
                ...
                try
                {
                    ...
                    {
                        MediaPlayer.GetInstance().currentAlbum = Utility.FindAlbum(songsToPrint[int.Parse(input) - 1].AlbumName);
                        MediaPlayer.GetInstance().currentPages.Clear();
                        MediaPlayer.GetInstance().currentPages.Add("m");
        ...
        public static string ArtistPage()
        {
            ...
            do
            {
                ...
                {
                    Console.WriteLine($"{j}. {artist.Albums[i].AlbumName} ");
                    j++;
                }
                ...
                try
                {
                    ...
                    {
                        MediaPlayer.GetInstance().currentAlbum = Utility.FindAlbum(artist.Albums[int.Parse(input) - topSongs.Count -1].AlbumName);
                        return "albumMenu";
                    }
                }
                catch (ArgumentOutOfRangeException)
        ...
        //            else if (int.Parse(input) >= 1 && int.Parse(input) <= songsToPrint.Count)
        //            {
        //                MediaPlayer.GetInstance().Play(Utility.FindSong(songsToPrint[int.Parse(input) - 1].Name));
        ...
        public static string Search(string SearchType = "")
        {
            ...
            else if (SearchType == "l")
            {
                ...
                {
                    if (album.AlbumName.ToLower() == input)
                    {
                        albums.Add(album);
                        Console.WriteLine(album.AlbumName);
                    }
            ...
            //                    MediaPlayer.GetInstance().currentPlaylist.AddSong(Utility.FindSong(songsToPrint[int.Parse(input) - 1].Name));
            //                    return "playlistMenu";
            //                }
            //            }
            //            catch (ArgumentOutOfRangeException)
        ...
        public static void SongPlayer()
        {
            ...
            if (MediaPlayer.GetInstance().currentSong != null)
            {
                ...
                if (MediaPlayer.GetInstance().isPlaying)
                {
                    ...
                    //Console.WriteLine($"In riproduzione: {MediaPlayer.GetInstance().currentSong.Name} di {MediaPlayer.GetInstance().currentSong.Artist.Name}");
                }
                else
                {
                    Console.BackgroundColor = ConsoleColor.DarkYellow;
                    //Console.WriteLine($"In pausa: {MediaPlayer.GetInstance().currentSong.Name} di {MediaPlayer.GetInstance().currentSong.Artist.Name}");
                }
            }
            Console.ResetColor();
        }
